...
const isLoading = computed(() =>
    createDataFileReferenceLoading.value ||
    updateDataFileReferenceLoading.value ||
...
const canSubmit = computed(() => {
    if (isEditing.value) {
        return formData.value.format && !schemaError.value && !isLoading.value
    }
    return formData.value.file && formData.value.format && !schemaError.value && !isLoading.value
...
const submitForm = async () => {
    ...
    try {
        ...
        console.error('Data file reference error:', error)
        uploadStatus.value = 'error'
...
const resetForm = () => {
    formData.value = { description: '', format: '', schema: '', file: null }
    formError.value = ''
    schemaError.value = ''
...
<template>
    <div class="data-file-reference-form">
