...
import {
  ENTITY_REFERENCE_TYPE_CONFIGS,
  isSelectableForEntity,
...
const { searchResults, searchLoading, searchError } = useReferencesSearchQuery({
  ...
  referenceTypes:ENTITY_REFERENCE_TYPE_CONFIGS.map(({key}) => key)
})

const { recentFileReferences, refetchRecentFiles } = useRecentFileReferencesQuery([REFERENCE_TYPE_CONFIGS.FILE.key])
