...
const { deleteProgram, deleteProgramLoading } = useMutatePrograms()
...
const submitDelete = async () => {
  try {
    ...
    const { status, errors } = await deleteProgram(props.program.id)

    if (status === 'SUCCESS') {
      emit('success')
    } else {
      deleteError.value = errors?.[0]?.message || 'Failed to delete program.'
    }
  } catch (error) {
  ...
  <div class="modal" @click.stop>
    ...
    <div class="modal-content">
      ...
      <div class="form-actions">
        ...
        >
          {{ deleteProgramLoading ? 'Deleting...' : 'Delete' }}
        </button>
        <button
          type="button"
          @click="$emit('close')"
