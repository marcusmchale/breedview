...
const { updateProgram, updateProgramLoading } = useMutatePrograms()

const formData = ref({
  name: props.program.name || '',
  fullname: props.program.fullname || '',
  description: props.program.description || ''
})

const formError = ref('')

// References state - initialize from program data
const selectedReferenceIds = ref(props.program.references?.map(r => r.id) || [])
const selectedReferences = ref(props.program.references || [])
...
const submitForm = async (values) => {
  try {
    ...
    if (status === 'SUCCESS') {
      ...
      formError.value = errors?.[0]?.message || 'Failed to update program.'
    }
  } catch (error) {
    ...
    <div class="modal-content">
      ...
      >
        ...
        <div class="form-actions">
          <button type="submit" class="btn btn-primary" :disabled="updateProgramLoading">
            {{ updateProgramLoading ? 'Updating...' : 'Update Program' }}
          </button>
